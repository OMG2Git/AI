...
class NewsAuthenticityChecker:
    def __init__(self):
        ...
        self.api_keys = {
            ...
            'gnews': '541932c8968b3ac9fd15dc4818e5f1dc',
...
def run_checker(url=None, text=None):
    ...
    llm_verdict = llm_data.get('final_verdict', '').upper()
    pipeline_verdict = pipeline_report.get('final_verdict', '').upper() if pipeline_report and 'final_verdict' in pipeline_report else None
    # --- Collect extra details for charting ---
    # LLM details
    try:
        groq_data = json.loads(groq_result.strip().replace('```json', '').replace('```', '')) if isinstance(groq_result, str) else groq_result
    except Exception:
    ...
    try:
        ...
        gemini_data = {}
    # Pipeline details
    emotion_analysis = pipeline_report.get('emotion_analysis', {}) if pipeline_report else {}
    content_analysis = pipeline_report.get('content_analysis', {}) if pipeline_report else {}
    source_analysis = pipeline_report.get('source_analysis', {}) if pipeline_report else {}
    fact_checking = pipeline_report.get('verification_results', []) if pipeline_report else []
    # Compose extra details
    extra_details = {
        'llm_details': {
            'groq': groq_data,
            'gemini': gemini_data
        ...
        'pipeline_details': {
            ...
            'article_info': pipeline_report.get('article_info', {}) if pipeline_report else {},
            'claims_verified': pipeline_report.get('claims_verified', 0) if pipeline_report else 0,
            'high_confidence_claims': pipeline_report.get('high_confidence_claims', 0) if pipeline_report else 0,
            'low_confidence_claims': pipeline_report.get('low_confidence_claims', 0) if pipeline_report else 0,
            'professional_indicators_count': pipeline_report.get('professional_indicators_count', 0) if pipeline_report else 0,
            'scientific_indicators_count': pipeline_report.get('scientific_indicators_count', 0) if pipeline_report else 0,
            'text_length': pipeline_report.get('text_length', 0) if pipeline_report else 0,
            'source_reliability': pipeline_report.get('source_reliability', 0) if pipeline_report else 0,
            'source_domain': pipeline_report.get('source_domain', '') if pipeline_report else '',
            'source_category': pipeline_report.get('source_category', '') if pipeline_report else '',
        }
    }
    if llm_verdict in ('REAL', 'FAKE') and pipeline_verdict in ('REAL', 'FAKE') and llm_verdict == pipeline_verdict:
        # Aggregate scores and merge features, output in abc.py format
        ...
        # Add extra details for charting
        combined_result.update(extra_details)
        return combined_result
    else:
        # Contradiction: prefer LLM output, but include both for transparency
        out_json = llm_data if isinstance(llm_json, dict) else json.loads(llm_json)
        out_json.update(extra_details)
        return out_json
